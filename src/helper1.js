...
export function getLinearIndex({row, col}, rows, cols) {
...
//this calculates each tile position in pixels
...
export function shuffle(tiles, rows, cols) {
    const shuffledTiles = [
        ...tiles.filter((t) => t !== tiles.length -1)
        .sort(()=> Math.random()- 0.5),
        tiles.length - 1,
    ];
    //this checks that is gives us a solvable puzzle and it's not giving already solved puzzle
    return isSolvable(shuffledTiles, rows, cols) && !isSolved(shuffledTiles)
    ? shuffledTiles : shuffle(shuffledTiles, rows, cols);
}

//this checks if we can place tile in empty slot(src = source, dest = destination)
...
//this function moves the tile into the empty slot
